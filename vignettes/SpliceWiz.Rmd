...
`Find_SpliceWiz_Output()`

```{r}
expr = Find_SpliceWiz_Output(irf_path)
```
...
suggested by SpliceWiz. `apply_filters` returns a vector containing which
...
There are more complex customisation options with SpliceWiz's Plot_Coverage tool
...
in the previous section, use the convenience function `Find_SpliceWiz_Output()`
to tabulate a list of samples and their corresponding IRFinder outputs:

```{r eval=FALSE}
expr = Find_SpliceWiz_Output("./SpliceWiz_Output")
```
