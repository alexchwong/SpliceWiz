...
<details>
<summary>What's New: Novel splice detection in version 0.99.3</summary>
\
...
* Additionally, it also compiles counts of "tandem junction" (hereafter TJ) 
which are reads that are split into 3 or more segments, compatible with the 
observation of excision of 2 or more introns in sequenced reads
* Novel junctions that share a common coordinate with that of an annotated
junction are retained, and are used to construct "novel transcripts" - which
are new transcripts that contain the observed junction. These are primarily
used to construct novel A5SS / A3SS events when paired with known junctions
* Novel TJ that share bounding coordinates (the two outermost coordinates)
with that of annotated TJs are retained, and are used to construct 
"novel transcripts" with two boundary exons flanking the observed "novel exon".
These are used to construct novel MXE / SE events involving cryptic exons.
...
"Annotation" subfolder of the output folder of `collateData()` function.

**TL/DR - how to enable novel ASE mode**

* To enable analysis involving both annotated and novel ASEs, simply set
`detectNovelSplicing = TRUE` when running `collateData()`. For example:
