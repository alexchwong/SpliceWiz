---
title: 'SpliceWiz: Workflow Explained'
author: "Alex CH Wong"
...
output:
    rmarkdown::html_document:
        ...
        toc_float: true
abstract:
    This vignette explains the SpliceWiz workflow in more detail. The runnable
    example contained here is identical to that in the Quick-Start vignette
    
    Version `r packageVersion("SpliceWiz")`
vignette: >
    %\VignetteIndexEntry{SpliceWiz: Workflow Explained}
...
This vignette is a detailed explaination of each step of a runnable working 
example of the SpliceWiz workflow.
...
The SpliceWiz package loads the `NxtIRFdata` data package, which contains the
working example: 
...
NB: for real genomes / transcriptomes, SpliceWiz generates a reference which
accounts for low mappability regions. These regions are empirically identified
using a method similar to that used by IRFinder. SpliceWiz provides human and
mouse "Mappability Exclusion" annotations via the `NxtIRFdata` package.

Assuming there is a genome file `"genome.fa"` and a transcript annotation
`"transcripts.gtf"` in the working directory, a SpliceWiz reference can be 
...
User-provided BED files can be provided via the `MappabilityRef` and
`nonPolyARef` arguments of the `buildRef()` function. Optionally, users can
provide a "blacklist" of regions via the `BlacklistRef` argument, as a BED file.
...
### Obtaining and locating the BAM files of the example dataset
...
# Copies SpliceWiz example BAM files to `bam_path` subdirectory
# - returns BAM paths
...
Using this data frame, collate the experiment using `collateData()`, which at
minimum, takes the following parameters:
...
We name the output directory as `NxtSE_output` as this folder will contain the 
data needed to import the NxtSE object:
...
(1) By default, `makeSE()` uses delayed operations to avoid consuming memory
...
(2) By default, `makeSE()` constructs the NxtSE object using all the samples in
...
(3) In complex transcriptomes including those of human and mouse, alternative
...
The NxtSE object inherits the SummarizedExperiment object. This means that the
functions for SummarizedExperiment can be used on the NxtSE object. These
include row and column annotations using the `rowData()` and `colData()`
accessors.

In the NxtSE object, each "row" represents a splicing event, whereas each
"column" represents a sample. We can annotate the samples using the 
`colData()` accessor, as follows:
...
### Filtering for high-confidence alternative splicing events (ASEs)
...
#### Retrieving SpliceWiz default filters

The default filters can be retrieved as follows:

```{r}
filterList <- getDefaultFilters()

filterList
```

For a detailed explanation of how these filters work, refer to the documentation
for the `ASEFilter` class:

```{r eval = FALSE}
?ASEFilter
```

#### Applying filters to the NxtSE object

The `applyFilters()` function takes two parameters as input. The first is the
parent NxtSE object. The second is the list of `ASEFilter` filters. It outputs
a vector of `TRUE`/`FALSE` values, which determines whether each splicing event
should be kept or discarded for downstream analysis.

By convenience, `applyFilters()` should be run within a row-subset operation, 
as follows:

```{r}
se.filtered <- se[applyFilters(se, filterList),]
```
