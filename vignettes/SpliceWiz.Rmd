...
output:
    rmarkdown::html_document:
        ...
        toc_float: true
abstract:
    SpliceWiz is an ultra-fast pipeline that quantifies splicing events using
    isoform-specific alignments. Alignment file processing is accelerated by its
    companion package ompBAM, a C++ library that automates OpenMP-based 
    multi-threaded file processing. Filtering of low confidence splicing events 
    enhances accuracy. Novel group-averaged sequencing coverage plots creates 
    powerful visualization of differential splicing between experimental groups.
    SpliceWiz is ideal for efficient exploration of alternative splicing in 
    large datasets, whereas ompBAM enables developers to create computationally
    efficient R packages to read alignment files.
...
Using these two files, we construct an example SpliceWiz reference as follows:
...
Finally, run `processBAM()` as follows:
...
### Collating processBAM output files into a SpliceWiz Experiment

First, collate the `processBAM()` output files using the helper function
`findSpliceWizOutput()`
...
This creates a 3-column data frame with sample name, gzipped text
...
The `collateData()` function creates arrays of junction read counts and intron
depths (depth of intron coverages). It creates an H5 dataset and Rds files
that enable import of the NxtSE object (see next section).
...
An experiment is a collection of samples in a dataset. Differential analysis
is all about comparing differences in splicing between differential experimental
groups. We can define these experimental conditions just like how it is done
using the `SummarizedExperiment` object (since `NxtSE` inherits this object).

Assign some example conditions to the NxtSE object and view these conditions
using the following:
...
The `ASE` family of functions perform differential expression of alternative
splicing events (ASEs). Counts can be modeled either using log-normal
distribution (using limma), negative binomial distribution with moderated
dispersions (using DESeq2), or beta-binomial distribution using DoubleExpSeq.

We find that for large experiments, DESeq2 is more computationally expensive. It
may be more expedient to use limma or DoubleExpSeq in these situations.
...
Now that we have a list of differential events, we can view the top events like
so:
...
Differential expression is often performed using log-fold change differences
which relies on a logarithmic scale. Often, many of these events have PSI's
that are very close to `0` or `1`. To obtain a list of differential events with
meaningful changes in PSI (on the linear scale), it is useful to apply an
additional filter requiring the delta-PSI to be greater than `0.05`:

```{r}
res_limma.filtered = subset(res_limma, abs(AvgPSI_A - AvgPSI_B) > 0.05)
```

#### Plotting a list (upto 4) of individual samples
...
Notice that the genomic axis can sometimes look crowded. To display only a
select number of transcripts, we can use the `selected_transcripts` argument
of the `plotCoverage()` function:
...
#### Plotting condition-averaged coverage plots

We can combine the coverages of individual samples. SpliceWiz does this by
normalizing the coverage depths of individual samples by their 
`transcript depth` (which is the sum of splice junction counts and retained 
intron estimates). The `tracks` argument is used to specify the condition names,
and the `condition` argument specifies the category name of the condition.
...
```{r, eval = FALSE}
se = makeSE(nxtse_path, RemoveOverlapping = TRUE)
```
