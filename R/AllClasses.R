...
#' Container to hold data to generate SpliceWiz coverage plots
#'
...
#' @param args A list of arguments used for data retrieval
#' @param annotation A list of data.tables containing region-specific annotation
#' @param colData A data.frame with the required annotation used to illustrate
#'   downstream plots
#' @param covData A list of RLEs containing retrieved raw coverage data
#' @param juncData A list, containing GRanges object of in-range junctions, 
#'   and matrices containing junction data (stranded, unstranded, and PSIs)
#' @param normData A list containing in-range Events and corresponding
#'   normalization factors
#' @return A covDataObject containing required data used to generate downstream 
#'   plots.
#' @examples
#'
#' # Note that covDataObject() is called internally using getCoverageData() and
#' # getGenomeData(), and is not meant to be called directly
#' covObj <- covDataObject()
#'
...
#' 
#' @param args A list of arguments used for data retrieval
#' @param cov A list of matrices containing raw or normalized coverage data
#' @param junc A list of GRanges objects containing raw counts or PSIs of
#'   splice junction counts, including plot heights of flanking exon boundaries
#' @param cov_stats A list of means and measures of variances (sd, sem, ci) of
#'   normalized coverages per track
#' @param diff_stats A data.frame containing negative log10 transformed
#'   p-values representing differences in normalized coverages (between tracks
#'   1 and 2)
#' @param annotation A list containing data.tables and other auxiliary data
#'   for plotting annotation tracks
#' @return A covPlotObject containing required data used to generate downstream 
#'   plots.
#' @examples
#'
#' # Note that covPlotObject() is called internally using ? and
#' # is not meant to be called directly
#' covPlotObj <- covPlotObject()
#'
