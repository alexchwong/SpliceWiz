...
the gene annotations. We can use `STAR_buildGenome` to do this:
...
Gene annotations can be injected, on-the-fly, at a later step. Additionally,
the `sjdbOverhang` parameter, and any extra FASTA files (such as ERCC standards,
...
This allows a single STAR reference to be built for each species, then these
can be adapted for different projects based on their specific technical
specifications (e.g. different read length can be adapted by setting different
`sjdbOverhang`, or any spike-ins by setting the spike-in FASTA using 
`extraFASTA`).
